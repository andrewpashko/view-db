...
final class DatabaseViewModel {
    ...
    var tableRows: [TableRowItem] {
        rowPage.rows.enumerated().map { offset, row in
            TableRowItem(id: rowPage.offset + offset, values: row)
        }
    }

    var sqlRows: [TableRowItem] {
        sqlRowPage.rows.enumerated().map { offset, row in
            TableRowItem(id: sqlRowPage.offset + offset, values: row)
        }
    }
    ...
    func selectTable(_ table: TableRef) {
        ...
        selectedTable = table
        fetchRows(limit: max(1, rowPage.limit), offset: 0)
    }

    func fetchNextPage() {
        guard rowPage.hasNext else { return }
        fetchRows(limit: max(1, rowPage.limit), offset: rowPage.offset + rowPage.limit)
    }

    func fetchPreviousPage() {
        guard selectedTable != nil else { return }
        let newOffset = max(0, rowPage.offset - max(1, rowPage.limit))
        fetchRows(limit: max(1, rowPage.limit), offset: newOffset)
    }
        ...
        do {
            ...
            if let selectedTable,
               let match = tables.first(where: { $0.id == selectedTable.id }) {
                ...
                fetchRows(limit: max(1, rowPage.limit), offset: rowPage.offset)
            } else if let first = tables.first {
                selectedTable = first
                fetchRows(limit: 200, offset: 0)
            } else {
                rowPage = .empty
            }
        } catch {
    ...
    private func fetchRows(limit: Int, offset: Int) {
        ...
        rowTask = Task { [weak self] in
            guard let self else { return }
            await self.fetchRowsImpl(table: selectedTable, limit: limit, offset: offset)
        }
    }

    private func fetchRowsImpl(table: TableRef, limit: Int, offset: Int) async {
        isLoadingRows = true
        errorMessage = nil
